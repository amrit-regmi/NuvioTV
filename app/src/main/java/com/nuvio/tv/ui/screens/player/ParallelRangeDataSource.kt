 ...
 *
 * Chunk downloads (completed AND in-flight) belong to a companion-level [ChunkSession] that
 * survives the close/open boundary ExoPlayer performs on every seek, so a non-faststart MP4's
 * scatter-read pattern (tail moov, front mdat) no longer pays a full close/reopen per hop.
 *
 * Hand-ported from upstream NuvioMedia (SHAs f5df26abb, 2c59865c1, e9f25fbec, abb0de74c,
 * cf00facfc, 10e5ca867, 8d587e1ae, cb4b405ba). Upstream's optional native/direct-buffer memory
 * path (DefaultAllocatorNative / ByteBufferDataReader) is dropped here — our forked media3
 * exoplayer AAR ships neither symbol — so chunk buffers stay on the Java heap as ByteArrays.
 */
@UnstableApi
internal class ParallelRangeDataSource(
    ...
    companion object {
        ...
        // A single, shared, lazy thread pool with bounded max threads to prevent OOM/pthread_create failure
        ...
        private val globalBufferPool = ConcurrentHashMap<Long, ConcurrentLinkedDeque<ByteArray>>()
        ...
        private fun releaseSessionBuffer(buffer: ByteArray, chunkSz: Long, poolCap: Int) {
            if (poolCap <= 0) return
        ...
        private fun evictFuture(
            ...
            if (!future.cancel(true) && future.isDone && !future.isCancelled) {
                try {
                    releaseSessionBuffer(future.get().data, session.chunkSize, poolCap)
                } catch (_: Exception) {
                }
            }
        }
        ...
        private fun clearGlobalPool() {
            globalBufferPool.values.forEach { pool -> pool.clear() }
        ...
        if (currentChunkIndex != chunkIndex || currentChunk == null) {
            ...
            try {
                ...
                if (activeSession.futures.remove(chunkIndex, future)) {
                    ...
                    if (!future.cancel(true) && future.isDone && !future.isCancelled) {
                        try {
                            releaseSessionBuffer(future.get().data, activeSession.chunkSize, maxPoolSize)
                        } catch (_: Exception) {
                        }
                    }
                }
    ...
    private fun acquireBuffer(): ByteArray {
        ...
        return pool.pollLast() ?: ByteArray(chunkSize.toInt())
    }
     ...
     *   is driven by the session chunk cap and the prefetch lookahead depth.
     */
